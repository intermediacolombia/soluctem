...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Modificar la consulta SQL para incluir `nombre`, `apellido`, `zonas`, `intentos_fallidos` y `ultimo_intento`
    ...
    if ($stmt->num_rows > 0) {
        // Vincular las variables, incluyendo `nombre`, `apellido`, `zonas`, `intentos_fallidos` y `ultimo_intento`
        ...
        if ($intentos_fallidos >= 5) {
            // Bloquear la cuenta
            $stmt = $conn->prepare("UPDATE usuarios SET activo = 0, ultimo_intento = NOW() WHERE id = ?");
            $stmt->bind_param("i", $id_usuario);
            $stmt->execute();

            // Mostrar mensaje de bloqueo
            $_SESSION['login_error'] = "Su usuario ha sido bloqueado. Por favor, contacte al administrador.";
        ...
        if (password_verify($password, $password_hash)) {
            ...
            if ($activo == 1) {
                // Reiniciar intentos fallidos y actualizar la fecha del último intento
                $stmt = $conn->prepare("UPDATE usuarios SET intentos_fallidos = 0, ultimo_intento = NOW() WHERE id = ?");
                $stmt->bind_param("i", $id_usuario);
                $stmt->execute();
