...
if (!empty($password) || !empty($confirmar_password)) {
    if ($password !== $confirmar_password) {
        ...
        $conn->close();
        header('Location: /admin/profile/edit_profile.php');
        exit();
    } else {
...
if (!empty($password)) {
    if ($imagen_perfil) {
        ...
        $stmt = $conn->prepare("UPDATE usuarios SET password = ?, nombre = ?, apellido = ?, correo_electronico = ?, telefono = ?, imagen_perfil = ? WHERE id = ?");
        $stmt->bind_param("ssssssi", $password_hash, $nombre, $apellido, $correo_electronico, $telefono, $imagen_perfil, $id_usuario);
    } else {
        // Actualizar incluyendo la contraseña sin cambiar la imagen de perfil
        $stmt = $conn->prepare("UPDATE usuarios SET password = ?, nombre = ?, apellido = ?, correo_electronico = ?, telefono = ? WHERE id = ?");
        $stmt->bind_param("sssssi", $password_hash, $nombre, $apellido, $correo_electronico, $telefono, $id_usuario);
    }
} else {
    if ($imagen_perfil) {
        // Actualizar incluyendo la imagen de perfil sin cambiar la contraseña
        $stmt = $conn->prepare("UPDATE usuarios SET nombre = ?, apellido = ?, correo_electronico = ?, telefono = ?, imagen_perfil = ? WHERE id = ?");
        $stmt->bind_param("sssssi", $nombre, $apellido, $correo_electronico, $telefono, $imagen_perfil, $id_usuario);
    } else {
        // Actualizar sin cambiar la contraseña ni la imagen de perfil
        $stmt = $conn->prepare("UPDATE usuarios SET nombre = ?, apellido = ?, correo_electronico = ?, telefono = ? WHERE id = ?");
        $stmt->bind_param("sssii", $nombre, $apellido, $correo_electronico, $telefono, $id_usuario);
    }
}
