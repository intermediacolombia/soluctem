...
if (!empty($password)) {
    if ($imagen_perfil) {
        // Actualizar incluyendo la contraseña, la imagen de perfil y reiniciar los intentos fallidos
        $stmt = $conn->prepare("UPDATE usuarios SET password = ?, rol = ?, zonas = ?, nombre = ?, apellido = ?, correo_electronico = ?, telefono = ?, activo = ?, imagen_perfil = ?, intentos_fallidos = 0 WHERE id = ?");
        ...
        // Actualizar incluyendo la contraseña y reiniciar los intentos fallidos sin cambiar la imagen de perfil
        $stmt = $conn->prepare("UPDATE usuarios SET password = ?, rol = ?, zonas = ?, nombre = ?, apellido = ?, correo_electronico = ?, telefono = ?, activo = ?, intentos_fallidos = 0 WHERE id = ?");
        $stmt->bind_param("sssssssii", $password_hash, $rol, $zonas, $nombre, $apellido, $correo_electronico, $telefono, $activo, $id_usuario);
    }
} else {
    if ($imagen_perfil) {
        // Actualizar incluyendo la imagen de perfil y reiniciar los intentos fallidos sin cambiar la contraseña
        $stmt = $conn->prepare("UPDATE usuarios SET rol = ?, zonas = ?, nombre = ?, apellido = ?, correo_electronico = ?, telefono = ?, activo = ?, imagen_perfil = ?, intentos_fallidos = 0 WHERE id = ?");
        $stmt->bind_param("ssssssisi", $rol, $zonas, $nombre, $apellido, $correo_electronico, $telefono, $activo, $imagen_perfil, $id_usuario);
    } else {
        // Actualizar reiniciando los intentos fallidos sin cambiar la contraseña ni la imagen de perfil
        $stmt = $conn->prepare("UPDATE usuarios SET rol = ?, zonas = ?, nombre = ?, apellido = ?, correo_electronico = ?, telefono = ?, activo = ?, intentos_fallidos = 0 WHERE id = ?");
        $stmt->bind_param("ssssssii", $rol, $zonas, $nombre, $apellido, $correo_electronico, $telefono, $activo, $id_usuario);
    }
}

if ($stmt->execute()) {
    $_SESSION['update_success'] = 'Datos actualizados correctamente y los intentos fallidos han sido reiniciados.';
    $stmt->close();
    $conn->close();
    header('Location: /admin/users/edit.php?id=' . $id_usuario);
    exit();
